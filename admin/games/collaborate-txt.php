<?php
$range = $database->get_assoc("SELECT * FROM `tcg_games_updater` WHERE `gup_set`='".$games->gameSet('collaborate-txt')."'");
$logChk = $database->get_assoc("SELECT * FROM `user_logs` WHERE `log_name`='$player' AND `log_title`='".$games->gameTitle('collaborate-txt')."' AND `log_date` >= '".$range['gup_date']."'");
...
if( isset( $_POST['submit'] ) )
{
	...
	else
	{
		...
		{
			echo '<h1>'.$games->gameTitle('collaborate-txt').' - Prize Pickup</h1>';
			echo '<center><p>Good work! Take everything you see below and don\'t forget to log it!</p>';
			$getWish = $database->get_assoc("SELECT * FROM `user_wishes` WHERE `wish_status`='Granted' AND `wish_date`='".$range['gup_date']."' AND `wish_set`='".$games->gameSet('collaborate-txt')."'");
			if( $getWish['wish_set'] == $games->gameSet( 'collaborate-txt' ) )
			{
				$choice = explode(", ", $games->gameChoiceArr('collaborate-txt'));
				$random = explode(", ", $games->gameRandArr('collaborate-txt'));
				$currency = explode(" | ", $games->gameCurArr('collaborate-txt'));
				foreach( $choice as $c ) { $cTotal = $c * 2; }
				foreach( $random as $r ) { $rTotal = $r * 2; }
				foreach( $currency as $m ) { $mTotal[] = $m * 2; }
				$mTotal = implode(" | ", $mTotal);
				$general->gamePrize($games->gameSet('collaborate-txt'),$games->gameTitle('collaborate-txt'),'('.$deckinfo['thm_deck'].')',$rTotal,$cTotal,$mTotal);
			}

			else
			{
				$cTotal = $games->gameChoiceArr('collaborate-txt');
				$rTotal = $games->gameRandArr('collaborate-txt');
				$mTotal = $games->gameCurArr('collaborate-txt');
				$general->gamePrize($games->gameSet('collaborate-txt'),$games->gameTitle('collaborate-txt'),'('.$deckinfo['thm_deck'].')',$rTotal,$cTotal,$mTotal);
			}
		}
	}
...
echo '<h1>'.$games->gameSet('collaborate-txt').' - '.$games->gameTitle('collaborate-txt').'</h1>
<table width="100%" cellspacing="0" cellpadding="0" border="0">
<tr>
	<td width="49%" valign="top">';
		echo $games->gameBlurb('collaborate-txt');
		echo '<br /><br /><center>';
		...
		if( $memrole == "1" )
		{
			...
			<form action="'.$tcgurl.'games.php?play=collaborate-txt" method="post">
	...
	<td width="49%" valign="top">';
		...
		while( $col = mysqli_fetch_assoc( $res ) )
		{
			...
			if( $memrole == "1" )
			{
				echo '<button class="btn-danger" onclick="location.href=\''.$tcgurl.'games.php?playe=collaborate-txt&action=delete&cat='.$col['thm_id'].'\'">Delete this deck</button>';
			}
			...
			for( $i = 1; $i <= $col['thm_count']; $i++ )
			{
				...
				else
				{
					...
					echo '<li>Card <b>#'.$digit.'</b> - <a href="'.$data[$i]['img'].'" target="_blank" title="'.$data[$i]['feat'].' of '.$data[$i]['series'].'">'.$data[$i]['user'].' ('.$data[$i]['feat'].' of '.$data[$i]['series'].')</a></li>';
				}
			}
			echo '</ul><br />';
			...
			if( $logDATE >= $ranDATE )
			{
				echo '<h3>'.$games->gameTitle('collaborate-txt').' : Halt!</h3>
				<center><p>You have already played this game! If you missed your rewards, here they are:</p>';
				$general->displayRewards('collaborate-txt');
				echo '</center>';
			}
			...
			{
				echo '<form action="'.$tcgurl.'games.php?play=collaborate-txt" method="post">
