...
If you are going to <u>add a password gate game</u>, <a href="'.$tcgurl.'admin/content.php?mod=games&action=add">use this form</a> instead.</p>
...
while( $row = mysqli_fetch_assoc( $sql ) )
{
	...
	<td align="center">';
		if( $row['game_status'] == "Inactive" )
		{
			echo '<button type="button" onClick="window.location.href=\''.$tcgurl.'admin/content.php?mod=games&action=activate&id='.$row['game_id'].'\';" title="Activate Game" class="btn btn-primary" data-toggle="tooltip" data-placement="bottom" title="Activate this game"><i class="bi-toggle-on" role="image"></i></button> ';
		}

		else
		{
			echo '<button type="button" onClick="window.location.href=\''.$tcgurl.'admin/content.php?mod=games&action=deactivate&id='.$row['game_id'].'\';" title="Deactivate Game" class="btn btn-warning" data-toggle="tooltip" data-placement="bottom" title="Deactivate this game"><i class="bi-toggle-off" role="image"></i></button> ';
		}

		if( empty( $row['game_pass_array'] ) )
		{
			echo '<button type="button" onClick="window.location.href=\''.$tcgurl.'admin/content.php?mod=games&action=edit-auto&id='.$row['game_id'].'\';" title="Edit Game" class="btn btn-success" data-toggle="tooltip" data-placement="bottom" title="Edit this game"><i class="bi-gear" role="image"></i></button> ';
		}

		else
		{
			echo '<button type="button" onClick="window.location.href=\''.$tcgurl.'admin/content.php?mod=games&action=edit&id='.$row['game_id'].'\';" title="Edit Game" class="btn btn-success" data-toggle="tooltip" data-placement="bottom" title="Edit this game"><i class="bi-gear" role="image"></i></button> ';
		}
		echo '<button type="button" onClick="window.location.href=\''.$tcgurl.'admin/content.php?mod=games&action=delete&id='.$row['game_id'].'\';" title="Delete Game" class="btn btn-danger" data-toggle="tooltip" data-placement="bottom" title="Delete this game"><i class="bi-trash3" role="image"></i></button>
	</td>
	</tr>';
}
