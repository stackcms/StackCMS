...
// Process edit an upcoming deck form
if( isset( $_POST['update'] ) )
{
	...
	$update = $database->query("UPDATE `tcg_cards` SET `card_filename`='$filename', `card_deckname`='$deckname', `card_donator`='$donator', `card_maker`='$maker', `card_color`='$color', `card_desc`='$desc', `card_set`='$set', `card_cat`='$cat', `card_count`='$cards', `card_worth`='$worth', `card_break`='$break', `card_mast`='$mast', `card_puzzle`='$puzzle', `card_masters`='$masters', `card_status`='$status', `card_released`='$date' WHERE `card_id`='$id'");
...
// Show edit an upcoming deck form
else
{
	$row = $database->get_assoc("SELECT * FROM `tcg_cards` WHERE `card_id`='$id' AND `card_status`='Upcoming'");
	$description = str_replace("<br />", " ", nl2br($row['card_desc']));

	if( $row['card_status'] != 'Upcoming' )
	{
		echo '<h1>Halt!</h1>
		<p>It seems like the deck that you are trying to edit is not an upcoming deck. Please make sure that the deck associated with an ID of '.$row['card_id'].' is an upcoming deck.</p>';
	}
	...
	else
	{
		...
		Use the <a href="'.$tcgurl.'admin/content.php?mod='.$mod.'&page='.$page.'&action=add">add</a> form to add new upcoming decks.</p>
        ...
        <div class="row">
			...
			<div class="col-4">
				<div class="box">
					...
					<div class="input-group mb-3">
						<div class="input-group-prepend">
							<span class="input-group-text"><b>Date Added</b></span>
						</div>
					...
					echo '<div align="right" style="margin-top:20px;">
						<input type="submit" name="update" class="btn btn-success" value="Edit Deck" /> 
