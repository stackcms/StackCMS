...
/**************************************
 * Module:			Deck Claims
 * Description:		Process user claims
 */


$month = date("m", strtotime("now"));
$max = $database->num_rows("SELECT count(*) FROM `tcg_donations` WHERE `deck_donator`='$player' AND MONTH(deck_date)='$month' GROUP BY `deck_date`");
...
if( $max < $settings->getValue( 'xtra_deck_cards' ) )
{
	if( isset($_POST['submit']) )
	{
		...
		// Insert data if new series is empty
		if( empty( $ser ) )
		{
			...
			if( !$insert )
			{
				$error[] = '<p>There was an error while processing your donations. Kindly send us your claim details instead at <u>'.$tcgemail.'</u>. Thank you and sorry for the inconvenience!</p> '.mysqli_error($insert);
			}
			else
			{
				$success[] = '<p>Your deck claim has been added to the database! Claim <a href="'.$tcgurl.'services.php?form=claims">more?</a></p>';
			}
		}
	...
	<tr>
		...
		<td width="35%">
			...
			<td width="35%"><input type="text" name="deckname" style="width:90%;" placeholder="e.g. samplecard">
		</td>
	</tr>
	<tr>
		<td><b>Feature:</b></td>
